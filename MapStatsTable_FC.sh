...
# alignment match
echo "3. Generating match count per read..."
cat $outdir'SamCigar' | sed 's/[0-9]*[!N]//g' | sed 's/[0-9]*[!S]//g' | sed 's/[0-9]*[!H]//g'| sed 's/[0-9]*[!P]//g' | sed 's/[0-9]*[!M]//g' | sed 's/[0-9]*[!X]//g' | sed 's/[0-9]*[!D]//g' | sed 's/[0-9]*[!I]//g' | sed 's/=//g'| awk '{c=0;for(i=1;i<=NF;++i){c+=$i};print c}' > $outdir$'CigarMatch'
paste $outdir'SamNames' $outdir$'CigarMatch' > $outdir'ReadNameCigarMatch'

# alignment mismatch
echo "4. Generating mismatch count per read..."
cat $outdir'SamCigar' | sed 's/[0-9]*[!N]//g' | sed 's/[0-9]*[!S]//g' | sed 's/[0-9]*[!H]//g'| sed 's/[0-9]*[!P]//g' | sed 's/[0-9]*[!M]//g' | sed 's/[0-9]*[!\=]//g' | sed 's/[0-9]*[!D]//g' | sed 's/[0-9]*[!I]//g' | sed 's/X//g'| awk '{c=0;for(i=1;i<=NF;++i){c+=$i};print c}' > $outdir$'CigarMismatch'
paste $outdir'SamNames' $outdir$'CigarMismatch' > $outdir'ReadNameCigarMismatch'


# mapped read length
echo "Calculating mapped read length (Matches & Insertions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarMatch') <(sort -k 1b,1 $outdir'ReadNameCigarIns') | awk -F='\t' '{print $1, ($2+$3)}' > $outdir'MapReadLength'

# reference length to which read hap been mapped
echo "Calculating reference length at mapping fragment (Matches & Deletions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarMatch') <(sort -k 1b,1 $outdir'ReadNameCigarDel') | awk -F='\t' '{print $1, ($2+$3)}' > $outdir'RefReadLength'

# perc alignment mtch read
echo "Calculating percentage of alignment match in mapped read length (Matches/Matches+Insertions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarMatch') <(sort -k 1b,1 $outdir'MapReadLength') | awk -F='\t' '!$3 {exit ; }{print $1, ($2/$3)}' > $outdir'AlnMatchPercRead'

# perc alignment mtch reference
echo "Calculating percentage of alignment match in mapped reference length (Matches/Matches+Deletions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarMatch') <(sort -k 1b,1 $outdir'RefReadLength') | awk -F='\t' '!$3 {exit ; } {print $1, ($2/$3)}' > $outdir'AlnMatchPercRef'

# perc alignment insertions read
echo "Calculating percentage of alignment match in mapped read length (Insertions/Matches+Insertions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarIns') <(sort -k 1b,1 $outdir'MapReadLength') | awk -F='\t' '!$3 {exit ; }{print $1, ($2/$3)}' > $outdir'AlnInsPercRead'

# perc alignment deletions reference
echo "Calculating percentage of alignment match in mapped reference length (Deletions/Matches+Deletions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarDel') <(sort -k 1b,1 $outdir'RefReadLength') | awk -F='\t' '!$3 {exit ; }{print $1, ($2/$3)}' > $outdir'AlnDelPercRef'

# merge basic stats
paste $outdir'MapReadLength' $outdir'RefReadLength' $outdir'AlnMatchPercRead' $outdir'AlnMatchPercRef' $outdir'AlnInsPercRead' $outdir'AlnDelPercRef' | awk -F='\t' '{print $1,$2,$4,$6,$8,$10,$12}' > $outdir'AlnCalc'
...
cat $outdir'AlnStats' | awk -F='\t' '!$2 {exit ; }{print $5/$2}' > $outdir'ReadAlnLen'
# mapped length to ref length
echo "Calculating proportion of mapped length to the reference length... "
cat $outdir'AlnStats' | awk -F='\t' '!$6 {exit ; }{print $5/$6}' > $outdir'ReadRefLen'
...
awk -F='\t' '{ sum += $2; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanReadLength'
# sd of read length
echo "Calculating sd of read length... "
awk -F='\t' '{sum+=$2; sumsq+=$2*$2} END {print sqrt(sumsq/NR - (sum/NR)**2)}'  $outdir'AlnStatsSpec' > $outdir'SdReadLength'

# average mapped read length
echo "Calculating mean of mapped read length... "
awk -F='\t' '{ sum += $5; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanMapReadLength'
# sd of read length
echo "Calculating sd of mapped read length... "
awk -F='\t' '{sum+=$5; sumsq+=$5*$5} END {print sqrt(sumsq/NR - (sum/NR)**2)}'  $outdir'AlnStatsSpec' > $outdir'SdMapReadLength'

# average mapped read length
echo "Calculating mean of mapped reference length... "
awk -F='\t' '{ sum += $6; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanMapRefLength'
# sd of read length
echo "Calculating sd of mapped reference length... "
awk -F='\t' '{sum+=$6; sumsq+=$6*$6} END {print sqrt(sumsq/NR - (sum/NR)**2)}'  $outdir'AlnStatsSpec' > $outdir'SdMapRefLength'

# average mapped read length
echo "Calculating mean of portion of mapped read on its total length... "
awk -F='\t' '{ sum += $11; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanReadtoReadRate'
# sd of read length
echo "Calculating sd of portion of mapped read on its total length... "
awk -F='\t' '{sum+=$11; sumsq+=$11*$11} END {print sqrt(sumsq/NR - (sum/NR)**2)}'  $outdir'AlnStatsSpec' > $outdir'SdReadtoReadRate'

# average mapped read length
echo "Calculating mean of portion of mapped read on mapped reference length... "
awk -F='\t' '{ sum += $12; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanReadtoRefRate'
# sd of read length
echo "Calculating sd of portion of mapped read on mapped reference length... "
awk -F='\t' '{sum+=$12; sumsq+=$12*$12} END {print sqrt(sumsq/NR - (sum/NR)**2)}'  $outdir'AlnStatsSpec' > $outdir'SdReadtoRefRate'

# average match in read length
echo "Calculating average percentage of mapping in mapped read length... "
awk -F='\t' '{ sum += $7; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanAlignmentMatchReadRate'

# average match in ref length
echo "Calculating average percentage of mapping in mapped reference length... "
awk -F='\t' '{ sum += $8; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanAlignmentMatchRefRate'

# average alignement insertion rate
echo "Calculating average percentage of insertions in mapped read length... "
awk -F='\t' '{ sum += $9; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanInsReadRate'

# average alignement insertion rate
echo "Calculating average percentage of deletions in mapped read length... "
awk -F='\t' '{ sum += $10; n++ } END { if (n > 0) print sum / n; }' $outdir'AlnStatsSpec' > $outdir'MeanDelReadRate'
