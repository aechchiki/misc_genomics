...
join <(sort -k 1b,1 $outdir'ReadNameCigarMatch') <(sort -k 1b,1 $outdir'ReadNameCigarIns') | awk '{print $1, ($2+$3)}' > $outdir'MapReadLength'

# reference length to which read hap been mapped
echo "Calculating reference length at mapping fragment (Matches & Deletions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarMatch') <(sort -k 1b,1 $outdir'ReadNameCigarDel') | awk '{print $1, ($2+$3)}' > $outdir'RefReadLength'

# perc alignment mtch read
echo "Calculating percentage of alignment match in mapped read length (Matches/Matches+Insertions)..."
join <(sort -k 1b,1 $outdir'ReadNameCigarMatch') <(sort -k 1b,1 $outdir'MapReadLength') | awk '!$3 {exit ; }{print $1, ($2/$3)}' > $outdir'AlnMatchPercRead'
...
join <(sort -k 1b,1 $outdir'ReadNameCigarMatches') <(sort -k 1b,1 $outdir'RefReadLength') | awk '!$3 {exit ; } {print $1, ($2/$3)}' > $outdir'AlnMatchPercRef'
